...
import { newArrivals, trendingProducts, topCategories } from '../database/shop';
        ...
        <div className={classes.shop__container}>
            <div className={classes.shop__content}>
                {newArrivals.map((newArrival) => {  
                    const {id, name, image, price} = newArrival;
                    return (
                        <div className={classes.shop__box} key={id}>
                            <div className={classes.shop__image}>
                                <img className={classes.shopImg} src={image} alt={name} />
                            </div>
                            <div className={classes.shop__details}>
                                <div className={classes.shopText}>{name}<br/>
                                    <span className={classes.shopPrice}>${price}</span>
                                </div>
                                <IconButton className={classes.shopIcon}>
                                    <ArrowRightAltIcon onClick={() => history.push(`/order/${id}`)}/>
                                </IconButton>  
                            </div>  
                        </div>
                    );
                ...
                {trendingProducts.map((trendingProduct) => {  
                    const {id, name, image, price, discount, oldprice} = trendingProduct;
                    return (
                        <div className={classes.shop__box} key={id}>
                            <div className={classes.shop__image}>
                                <img className={classes.shopImg} src={image} alt={name} />
                            ...
                            <div className={classes.shop__details}>
                                <div className={classes.shopText}>{name}<br/>
                                    <span className={classes.shopPrice}>${oldprice}</span>
                                    <span className={classes.shopPrice}>${discount}</span>
                                    <span className={classes.shopPrice}>${price}</span>
                                </div>
                                <IconButton className={classes.shopIcon}>
                                    <ArrowRightAltIcon onClick={() => history.push(`/order/${id}`)}/>
                ...
                {topCategories.map((topCategory) => {  
                    const {id, name, image, price, oldprice} = topCategory;
                    return (
                        <div className={classes.shop__box} key={id}>
                            <div className={classes.shop__image}>
                                <img className={classes.shopImg} src={image} alt={name} />
